<html>
<head>
	
	<title>
		Club Registration Form
	</title>

</head>
<body>
	<br><br><br>
	<h1>
			CLUB REGISTRATION FORM
	</h1>
	
	<p>
	<br><br>
	<form>
	<pre>
		Name		: <input type="text"  name="name">		<br>
		Address		: <input type="text" name="address"> 		<br>
		Mobile		: <input type="text" name="mobile" >		<br>
		
		Member Type	:  <input type="radio" name="type" value="temp"> Temporary
				   <input type="radio" name="type" value="temp"> Permanent	<br>
			
		Date of birth	: Date	<select name="date">
				<option value="1"> 1		<option value="2"> 2		<option value="3"> 3		<option value="4"> 4
				<option value="5"> 5		<option value="6"> 6		<option value="7"> 7		<option value="8"> 8
				<option value="9"> 9 		<option value="10"> 10	<option value="11"> 11	<option value="12"> 12
				<option value="13"> 13	<option value="14"> 14	<option value="15"> 15	<option value="16"> 16
				<option value="17"> 17	<option value="18"> 18	<option value="19"> 19 	<option value="20"> 20
				<option value="21"> 21	<option value="22"> 22	<option value="23"> 23	<option value="24"> 24
				<option value="25"> 25	<option value="26"> 26	<option value="27"> 27	<option value="28"> 28
				<option value="29"> 29 	<option value="30"> 30	<option value="31"> 31
			</select>	 <br>
				  Month <select name="month">
				<option value="01"> 1		<option value="02"> 02	<option value="03"> 03	<option value="04"> 04
				<option value="05"> 05	<option value="06"> 06	<option value="07"> 07	<option value="08"> 08
				<option value="09"> 09 	<option value="10"> 10	<option value="11"> 11	<option value="12"> 12
			</select ><br>
				  Year <select name="year">
				<option value="1"> 1991	<option value="2"> 1992	<option value="3">1993	<option value="4"> 1994
				<option value="5"> 1995	<option value="6"> 1996	<option value="7"> 1997	<option value="8"> 1998	
				<option value="9"> 1999 	<option value="10"> 2000	<option value="11"> 2001	<option value="12"> 2002
			</select>
			<br>
		Reference	: <select name="reference">
				<option value="relative"> Relative 	<option value=" friend"> Friend
				<option value="google"> Google
			</select>
			<br><br>
			<input type="button" name="save" value="sumbit"> 	<input type="button" name="cancel" value="cancel">
		</pre>
	</form>
	</p>
	</font>
</body>
</html>
		
